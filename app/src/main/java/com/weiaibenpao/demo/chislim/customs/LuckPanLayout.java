...
public class LuckPanLayout extends View {
    ...
    private void drawSmallCircle(boolean FirstYellow){
        int pointDistance = radius - SpanUtil.dip2px(context,10);
        for(int i=0;i<=360;i+=20){
            int x = (int) (pointDistance * Math.sin(SpanUtil.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(SpanUtil.change(i))) + CircleY;

            if(FirstYellow)
                canvas.drawCircle(x,y,SpanUtil.dip2px(context,4),yellowPaint);
            else
                canvas.drawCircle(x,y,SpanUtil.dip2px(context,4),whitePaint);
